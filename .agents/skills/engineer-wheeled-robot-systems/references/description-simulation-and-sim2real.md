...
若用户指定采用官方或第三方参考模型包装仿真，不得静默把原始底盘事实改写为该模型。保留原始输入，在 `generation_trace.json` 中以 `overridden` 记录用户授权、替代模型、几何/轮数/接口影响和合同证据。参考模型只证明其仿真范围，不证明用户实车的几何、传感器或安全行为。
...
实际启动后再写 `runtime_observation.json` 并运行 `validate_runtime_observation.py --require-ran`。对每个 `active` 且 `in_critical_flow` 的组件，观测记录必须证明其输入输出连接到关键路径；启动但输出无人消费的平滑器、滤波器或安全器是失败，不得因为最终底盘仍能运动而忽略。`headless` 与 `gui` 是不同 profile：分别记录 RTF、墙钟频率、采样窗口和资源观测。用户要求的相机、图像传输、GUI/RViz 或其他 profile 必须列入该 profile 的 `required_topics`；缺少实测帧率/证据时声明 `partial`，完整验收使用 `--require-complete`。配置的 `update_rate`、传感器频率或发布率不是验收测量值。

任务型仿真还要保存 `action_trace.json`，并在 `runtime_observation.json` 记录 Action client/server。每个 terminal action 必须有 goal ID、来源、终态和结束时间；`aborted` 需要错误码/消息，`canceled` 需要取消来源。有限任务在终态后保留 spin 时，用受控关闭记录替代“进程退出码必须为零”的错误要求。仅有 lifecycle 为 active 或日志中未见 traceback 不能证明任务完成。急停、超时和健康门测试必须在非零运动期间触发并写 `fault_injection_trace.json`。详细 schema 与 L2 回归最小集见 [runtime-evidence-and-regression.md](runtime-evidence-and-regression.md)。
