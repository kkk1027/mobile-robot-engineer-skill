...
2. 收集会改变方案的最小信息。读取 [system-intake-and-routing.md](references/system-intake-and-routing.md)。记录硬件、技术栈、项目方向、执行边界、每项事实来源，以及用户明确授权的模型/参数/接口偏差。
...
| G1 | 本地构建、仿真、单元/集成测试；经明确授权的隔离仿真 VM | 可直接执行；远程 VM 必须记录用户授权、主机范围和项目目录，且不得借此访问实机或外部账户 |
...
1. 冻结结构化输入及哈希，维护 `generation_trace.json`。每个事实只能标为 `used`、`question`、`blocked`、`intentionally_unused` 或 `overridden`；`used` 必须有产物证据。`overridden` 必须有用户明确授权记录、原因、替代值、影响和产物证据。
...
7. L2 运行后记录 `runtime_observation.json`：实际节点/生命周期、发布订阅者、Action client/server、TF 所有者、QoS、墙钟频率、RTF、diagnostics 和 profile。把必测传感器、相机与 GUI profile 放进 `required_topics`；缺失时交付只能是 `partial`/`planned`，不能删项冒充完整。
8. 含导航、巡检、探索、回充或跟随任务时记录 `action_trace.json`，关联 goal、来源、取消、错误码、恢复和最终状态；有限任务在 terminal action 后仍 spin 时，记录受控关闭证据，不能只以“节点仍 active”或信号退出码判定任务成功。
9. 为 L2 交付提供启动、TF/传感器、唯一最终命令/超时、任务 action、运动中故障注入和性能阈值的自动集成测试。急停/超时必须在观察到非零运动时触发，并记录停止时延到 `fault_injection_trace.json`；运行环境不可用时生成测试并标为 `planned`，不得声称已通过。
10. 交付或盲测时生成 `evidence_manifest.json`，对随包 JSON、日志、探针和截图计算 SHA-256；扫描所有引用的证据路径，避免“报告引用了但交付包没有”的伪可复核结论。

使用 `validate_project_intake.py`、`validate_generation_trace.py`、`validate_robot_contract.py`、`validate_runtime_graph.py`、`validate_runtime_observation.py`、`validate_action_trace.py`、`validate_fault_injection.py` 和 `validate_evidence_bundle.py`。运行证据规则读取 [runtime-evidence-and-regression.md](references/runtime-evidence-and-regression.md)。硬件合同必须同步传播到 URDF/Xacro、控制器、固件、状态估计、仿真和启动配置。
...
- `validate_runtime_observation.py`：检查实际运行 profile 的关键流、Action client/server、活跃组件处置、动态 TF 所有者、观测频率、必测接口覆盖和 RTF。
- `validate_action_trace.py`：检查导航/探索/跟随等 action 的来源、终态、取消、错误和受控关闭证据。
- `validate_fault_injection.py`：检查运动中急停、超时或健康故障触发后的安全停止与时延证据。
- `validate_evidence_bundle.py`：检查交付证据的 SHA-256 以及 JSON 引用的工件是否随包可复核。
...
完成不是“每个包能编译”或“所有节点 active”。只有当目标需求具备可追溯实现和验收证据、设计图和运行事实都闭合、必测 profile/传感器覆盖完整、关键 action 有终态、命令能在运动中安全停止、性能结论来自实测 profile、证据包可复核、实机等级与证据一致时，才可以声明整机功能完成。未验证能力明确标为 `planned`、`partial` 或 `blocked`。
